...
from.model_blocks import BackboneModulation, SpectralModulation
...
class AttentionBlock(nn.Module):
    ...
    def __init__(
        ...
        self.norm = nn.GroupNorm(num_channels=num_channels,num_groups=norm_num_groups, eps=norm_eps, affine=True)
...
class ResnetBlock(nn.Module):
    ...
    def __init__(
        ...
        self.norm1 = AdaGroupNorm(num_groups=norm_num_groups, out_dim=in_channels, embedding_dim=temb_channels,eps=norm_eps)
        ...
        self.norm2 = AdaGroupNorm(num_groups=norm_num_groups, out_dim=out_channels, embedding_dim=temb_channels,eps=norm_eps)
...
class DiffusionModelUNet(nn.Module):
    def __init__(
        ...
        self.num_pos_embeds = num_pos_embeds

        self.conv_in = Convolution(
            spatial_dims=spatial_dims, in_channels=in_channels, out_channels=num_channels[0],
        ...
        if num_pos_embeds is not None:
            self.pos_proj = nn.Sequential(
                nn.Linear(num_pos_embeds, time_embed_dim), nn.SiLU(), nn.Linear(time_embed_dim, time_embed_dim)
            )
        ...
        for i in range(len(num_channels)):
            ...
            self.down_blocks.append(get_down_block(
                spatial_dims=spatial_dims, in_channels=input_channel, out_channels=output_channel,
                temb_channels=time_embed_dim, num_res_blocks=num_res_blocks[i],
                norm_num_groups=norm_num_groups, norm_eps=norm_eps, add_downsample=not is_final_block,
                resblock_updown=resblock_updown, with_attn=(attention_levels[i] and not with_conditioning),
                with_cross_attn=(attention_levels[i] and with_conditioning),
                num_head_channels=ensure_tuple_rep(num_head_channels, len(num_channels))[i],
                transformer_num_layers=transformer_num_layers, cross_attention_dim=cross_attention_dim,
                upcast_attention=upcast_attention, use_flash_attention=use_flash_attention, dropout_cattn=dropout_cattn
            ))

        # Mid Block
        self.middle_block = get_mid_block(
            spatial_dims=spatial_dims, in_channels=num_channels[-1], temb_channels=time_embed_dim,
            norm_num_groups=norm_num_groups, norm_eps=norm_eps, with_conditioning=with_conditioning,
            num_head_channels=ensure_tuple_rep(num_head_channels, len(num_channels))[-1],
            transformer_num_layers=transformer_num_layers, cross_attention_dim=cross_attention_dim,
            upcast_attention=upcast_attention, use_flash_attention=use_flash_attention, dropout_cattn=dropout_cattn
        )
        ...
        rev_channels = list(reversed(num_channels))
        output_channel = rev_channels[0]
        for i in range(len(rev_channels)):
            prev_out = output_channel
            output_channel = rev_channels[i]
            input_channel = rev_channels[min(i + 1, len(num_channels) - 1)]
            self.up_blocks.append(get_up_block(
                spatial_dims=spatial_dims, in_channels=input_channel, prev_output_channel=prev_out,
                out_channels=output_channel, temb_channels=time_embed_dim,
                num_res_blocks=list(reversed(ensure_tuple_rep(num_res_blocks, len(num_channels))))[i] + 1,
                norm_num_groups=norm_num_groups, norm_eps=norm_eps, add_upsample=i != len(num_channels) - 1,
                resblock_updown=resblock_updown, with_attn=(list(reversed(attention_levels))[i] and not with_conditioning),
                with_cross_attn=(list(reversed(attention_levels))[i] and with_conditioning),
                num_head_channels=list(reversed(ensure_tuple_rep(num_head_channels, len(num_channels))))[i],
                transformer_num_layers=transformer_num_layers, cross_attention_dim=cross_attention_dim,
                upcast_attention=upcast_attention, use_flash_attention=use_flash_attention, dropout_cattn=dropout_cattn
            ))
        ...
        self.out = nn.Sequential(
            ...
            nn.SiLU(),
            Convolution(spatial_dims=spatial_dims, in_channels=num_channels[0], out_channels=out_channels, 
                        strides=1, kernel_size=3, padding=1, conv_only=True)
        )

        self.backbone_mod_64 = BackboneModulation()
        self.skip_mod_64 = SpectralModulation(base_size=(32, 32, 32))

        self.backbone_mod_128 = BackboneModulation()
        self.skip_mod_128 = SpectralModulation(base_size=(16, 16, 16))
    ...
    def forward(
        ...
        # 2. Position/Class embedding
        if self.num_pos_embeds is not None and position_labels is not None:
            emb = emb + self.pos_proj(position_labels).to(dtype=x.dtype)
        ...
        for upsample_block in self.up_blocks:
            ...
            if h.shape[1] == 64:
                h = self.backbone_mod_64(h)
                fft_sample = [self.skip_mod_64(s) for s in res_samples]
            elif h.shape[1] == 128:
                h = self.backbone_mod_128(h)
                fft_sample = [self.skip_mod_128(s) for s in res_samples]
            else:
                fft_sample = res_samples

            h = upsample_block(hidden_states=h, res_hidden_states_list=fft_sample, temb=emb, context=context)
